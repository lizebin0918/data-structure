...
import com.lzb.map.test.model.*;

public class Main {
	

	static void test2(MyHashMap<Object, Integer> map) {
		for (int i = 1; i <= 20; i++) {
			map.put(new Key(i), i);
		}
		for (int i = 5; i <= 7; i++) {
	...
	static void test3(MyHashMap<Object, Integer> map) {
		map.put(null, 1); // 1
	...
	static void test5(MyHashMap<Object, Integer> map) {
		for (int i = 1; i <= 10; i++) {
